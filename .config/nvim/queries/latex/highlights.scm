...
; TODO: These highlights are out of sync with the compiled parser.
;   The compiled parser has curly_group_label/curly_group_label_list node types,
;   but the queries use curly_group_text/curly_group_text_list for label-
;   related patterns — a version mismatch between when the parser was compiled
;   and when the queries were copied. 
;   The options are either recompile the parser from the same commit as the 
;   queries, or update the affected lines to use the correct node types.
;
; (label_definition
